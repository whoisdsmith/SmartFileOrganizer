"""
Cache Manager for VideoAnalyzerPlugin.

Provides caching functionality to store and retrieve video analysis results,
...
logger = logging.getLogger("AIDocumentOrganizerV2.VideoAnalyzer.Cache")

class VideoAnalysisCache:
    """
    Manages caching of video analysis results to avoid redundant processing.
    """
    
    def __init__(self, cache_dir: Optional[str] = None):
        """
        Initialize the video analysis cache.
        
        Args:
            cache_dir: Directory to store cache files (default: ~/.ai_doc_organizer/cache/video)
        """
        if cache_dir is None:
            # Default cache directory in user's home folder
            home_dir = os.path.expanduser("~")
            self.cache_dir = os.path.join(home_dir, ".ai_doc_organizer", "cache", "video")
        else:
        ...
        logger.info(f"Video analysis cache initialized at: {self.cache_dir}")
    ...
    def clear(self) -> bool:
        ...
        try:
            ...
            logger.info("Cleared video analysis cache")
            return True
